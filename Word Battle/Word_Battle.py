...
def game_manual() -> None:
    """View the game manual."""
    def display_paths(board_length: int, starting_position: Tuple[int, int]) -> None:
        """Display all the paths."""
        b = Board()
        b.create_board(board_length)
        b.starting_position = tuple([n - 1 for n in starting_position])
        b.create_valid_paths()
        temp_board = np.full((b.length, b.length), " ", dtype='U1')
        temp_colour_map = b.set_colour_map()
        ...
        b.display_board(temp_board, temp_colour_map)

    def display_selected_path(board_length: int, starting_position: Tuple[int, int], path_number: int) -> None:
    ...
    def display_placed_word(board_length: int, starting_position: Tuple[int, int], path_number: int, word: str) -> None:
        """Display the placed word."""
        b = Board()
        b.create_board(board_length)
        b.starting_position = tuple([n - 1 for n in starting_position])
        b.create_valid_paths()
        b.selected_path = b.paths_full[path_number - 1]
        b.place_word(word)
        b.display_board()

    def heading(name) -> None:
        """Create a heading."""
    ...
    def page4() -> None:
        ...
        heading("Rules")
        print("Players:")
        print("1. One word per turn for each player.")
        print("\nPath Selections:")
        print("2. Only one starting position can be selected.")
        print("3. Full paths cannot be selected.")
        print("\nWord Placements:")
        print("4. Words must be placed at edges or corners.")
        print("5. Words must be equal in length with the selected path before it can be placed.")
        print("6. Words must contain letters that are in the selected path before it can be placed.")
        print("7. Words cannot overwrite other words.")
        print("\nWords:")
        print("8. Words can only be used from the accepted language dictionary.")
        print("9. Words must not contain numbers and punctuation marks.")
    ...
    def page3(board_length: int) -> None:
        """Contents of page 3."""
        clear_screen(0)
        ...
        print("\nResigning the Game:")
        print("If an appropriate word cannot be found, type 0 in any input to resign.")
        print("-----------------------------------------------------------------------------------------------------<page 3>------------------------------------------------------------------------------------------------------")
        user_input = input("                                                                      Press any key to continue to next page. Type 0 to go back to main menu: ")
    ...
    def page2(board_length: int) -> None:
        """Contents of page 2."""
        clear_screen(0)
        heading("Gameplay (cont.)")
        ...
        for i in range(board_length):
            b.colour_map[0, i] = colour
            b.colour_map[i, 0] = colour
        ...
        display_paths(board_length, (1, 1))
        print("Starting position at a edge.")
    ...
    def page1() -> None:
        ...
        print("Word Battle is a strategic word game situated around two or more players playing on a square board arbitrary size between 3 and 15 with the objective of disabling every player from placing a word down onto the\nboard to be the last player standing inorder to win the game.\n")
        heading("Setup")
        print("Players:")
        print("Decide on who is going to go first. The player who inputs their name first will be first while the rest follows.")
        print("\nBoard:")
        print("Decide the size of the board.")
        print(f"The smallest size is {LOWER_LIMIT} and the largest size is {UPPER_LIMIT}.")
        print("The board will always be square shaped.")
        print("\nLanguage:")
        print("English is the only language that can be used and do not replace the language text file that came with the game.\n")
        heading("Gameplay")
        print("Overview:")
        b = Board()
        board_length = 5
        ...
        print("To begin the game, start by selecting a starting position. Then you will be able to select a path which is where the word will be placed.\n")
        print("\n\n\n\n\n\n\n\n")
        print("-----------------------------------------------------------------------------------------------------<page 1>------------------------------------------------------------------------------------------------------")
        user_input = input("                                                                      Press any key to continue to next page. Type 0 to go back to main menu: ")
...
class Game:
    ...
    def __init__(self, starting_counter: int, length: int, players: List[Dict[str, str]]) -> None:
            ...
            if num_of_players == 0:
                ...
                if self_play:
                    for i in range(num_of_players):
                        self.players.append({"name": f"{COMPUTER_PLAYER_NAME} {i + 1}", "type": "computer", "difficulty": self.get_difficulty(f"{COMPUTER_PLAYER_NAME} {i + 1}"), "stats": {"wins": 0, "loses": 0, "draws": 0}})
                else:
                    clear_screen(0)
                    ...
                    if vs_computer:
                        if num_of_players > 2:
                            ...
                            self.players.append({"name": f"{COMPUTER_PLAYER_NAME}", "type": "computer", "difficulty": self.get_difficulty(COMPUTER_PLAYER_NAME), "stats": {"wins": 0, "loses": 0, "draws": 0}})
                    elif not vs_computer:
                        while num_of_players - 1 > 0:
                            clear_screen(0)
                            player = {"name": self.get_player_name(), "type": "human", "difficulty": None, "stats": {"wins": 0, "loses": 0, "draws": 0}}
                            self.players.append(player)
                            num_of_players -= 1
